...
TEST(simpleFileSystem,addValid) { // adds two different files to the fileSystem, expects success as return value both times
    // REDIRECT COUT STREAM -- TO PROTECT AGAINST ERRORS
    streambuf* backup_out;
    backup_out = cout.rdbuf();
    stringstream ss_out;
...
TEST(simpleFileFactory,createFileValid) { // creates two files by calling create on the file fileFactory, the calls should return the new file objects that are both not null
    // REDIRECT COUT STREAM -- TO PROTECT AGAINST ERRORS
    streambuf* backup_out;
    backup_out = cout.rdbuf();
    stringstream ss_out;
...
TEST(simpleFileFactory,createUnknownExtension) { // when a bad extension is passed, the fileFactory should pass back a nullptr
    // REDIRECT COUT STREAM -- TO PROTECT AGAINST ERRORS
    streambuf* backup_out;
    backup_out = cout.rdbuf();
    stringstream ss_out;
...
TEST(lScommand,getAllFiles) { // condirms set returned by getFileNames() matches the file fileSystem
    // REDIRECT COUT STREAM -- TO PROTECT AGAINST ERRORS
    streambuf* backup_out;
    backup_out = cout.rdbuf();
    stringstream ss_out;
